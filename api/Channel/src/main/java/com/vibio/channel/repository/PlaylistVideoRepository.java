...
public interface PlaylistVideoRepository extends JpaRepository<PlaylistVideo, String> {
	...
	@Query(
			"select v from PlaylistVideo v where v.playlist.id = :playlistId and v.playlist.channel.accountId = :accountId")
	Page<PlaylistVideo> findByPlaylistIdAndAccountId(String playlistId, String accountId, Pageable pageable);

	@Modifying
	@Query(
			"update PlaylistVideo  v set v.order = v.order + 1 where v.playlist.id = :playlistId and v.order >= :start and v.order <= :end")
