...
public class StudioPlaylistController {
	...
	public ApiResponse<?> createPlaylist(
			@RequestBody @Valid PlaylistRequest request, @AuthenticationPrincipal AuthenticatedUser user) {
	...
	public ApiResponse<?> getPlaylistById(
			@PathVariable("playlistId") String playlistId, @AuthenticationPrincipal AuthenticatedUser user) {
	...
	public ApiResponse<?> updateVideoOrder(
		...
		return ApiResponse.success(playlistVideoService.updateVideoOrder(user.getId(), playlistId, request));
	}
