...
public interface VideoReactionRepository extends JpaRepository<VideoReaction, VideoReactionKey> {
	...
	@Query(
			"select count(*) from VideoReaction reaction where  reaction.id.videoId = :videoId and reaction.type = 'LIKE'")
	Integer countLikeByVideoId(String videoId);

	@Query(
			"select count(*) from VideoReaction reaction where  reaction.id.videoId = :videoId and reaction.type = 'DISLIKE'")
