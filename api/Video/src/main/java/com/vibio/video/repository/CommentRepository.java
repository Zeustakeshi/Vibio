...
public interface CommentRepository extends JpaRepository<Comment, String> {
    ...
    @Query("select c from Comment c where  c.video.id = :videoId and c.parent is null ")
    Page<Comment> getAllComment(String videoId, Pageable pageable);

    @Query(
            "select c from Comment c where  c.video.id = :videoId and c.video.visibility = 'PUBLIC' and c.parent is null ")
    Page<Comment> getAllGuestComment(String videoId, Pageable pageable);

    @Query("select c from Comment c where  c.parent.id = :parentId and c.video.id = :videoId")
    Page<Comment> getAllReplies(String videoId, String parentId, Pageable pageable);

    @Query(
            "select c from Comment c where  c.parent.id = :parentId and c.video.id = :videoId and c.video.visibility = 'PUBLIC'")
